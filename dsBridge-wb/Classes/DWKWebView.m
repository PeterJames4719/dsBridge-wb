...
completionHandler:(void (^)(NSString * _Nullable result))completionHandler
{
    ...
    NSLog(@"dsbridge prompt->%@", prompt);
    //NSLog(@"dsbridge defaultText->%@", defaultText);
...
- (bool) hasNativeMethod:(NSDictionary *) args
{
    ...
    NSString *typeString = args[@"type"];
    if (![typeString isKindOfClass:[NSString class]]) {
        return false;
    }
    NSString * type= [typeString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    id JavascriptInterfaceObject= [javaScriptNamespaceInterfaces valueForKey:nameStr[0]];
    if(JavascriptInterfaceObject){
